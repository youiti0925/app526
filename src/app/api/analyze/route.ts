...
export async function POST(request: NextRequest) {
  try {
    ...
    const { frames, apiKey, projectName, projectCategory, transcription, videoContext, glossary } = body;
    ...
    // Add worker-provided context
    if (videoContext) {
      contextPrompt += `\n\n=== 作業者からの補足情報 ===`;
      if (videoContext.workDescription) {
        contextPrompt += `\n作業の概要・目的: ${videoContext.workDescription}`;
      }
      if (videoContext.knowledgeTips) {
        contextPrompt += `\n作業者のコツ・暗黙知: ${videoContext.knowledgeTips}`;
      }
      if (videoContext.safetyPoints) {
        contextPrompt += `\n安全上の注意点: ${videoContext.safetyPoints}`;
      }
      if (videoContext.workerNotes) {
        contextPrompt += `\nその他メモ: ${videoContext.workerNotes}`;
      }
      contextPrompt += `\n※ 上記の補足情報を分析結果に必ず反映してください。特にコツ・暗黙知はkeyPointsに、安全注意点はsafetyNotesに含めてください。`;
    }

    // Add audio transcription
    if (transcription) {
      contextPrompt += `\n\n=== 動画音声の文字起こし ===\n${transcription}\n※ この音声情報も分析に活用してください。作業者の発言や指示を手順に反映してください。`;
    }

    // Add glossary
    if (glossary) {
      contextPrompt += glossary;
    }
