...
export type Capability =
  ...
  | "direction" // 演出・構成のセンス
  | "music"
  | "physical_goods" // 布・木・樹脂などを加工して現物を送る
  | "personal_identity" // 本人の身元・声・顔・実績・アカウントが要る
  | "onsite";
...
export const CAPABILITIES: CapabilityDef[] = [
  ...
  },
  {
    ...
    needsPaid: false,
    detail:
      "本人の声・顔・実績・アカウント・身元が成果物の一部になっている仕事です。" +
      "代わりに作ることができません。名前を借りて出すのは、依頼者を欺くことになります。",
    patterns: [
      // 自分の声・顔で出る
      /(ご?自身|あなた)の(声|お声|顔|写真|画像|アカウント|SNS|インスタ|X)/,
      /(顔出し|声出し|自撮り|本人確認|身分証)/,
      /(音声|画面)(を)?(収録|録画)[^。\n]{0,10}(していただ|お願い|できる方|環境)/,
      // 本人の実績・体験が商品
      /(実体験|体験談|経験者|実績のある方)[^。\n]{0,8}(限定|のみ|募集|お願い)/,
      /(お話|知見|ノウハウ)[^。\n]{0,10}(聞かせて|伺|教えて)[^。\n]{0,8}(ください|いただ|欲しい)/,
      /(インタビュー|ヒアリング)(を)?(受け|お受け)/,
      /(壁打ち|相談に乗|アドバイスをいただ)/,
      // 本人が現場で体験する
      /(モニター|覆面調査|体験していただく)/,
      // 応募時に本人の情報を出させる
      /(ポートフォリオ|ご経歴|自己紹介動画|アカウントURL)[^。\n]{0,10}(ご?提示|提出|教えて|お知らせ)/,
    ],
  },
  {
    id: "physical_goods",
    ...
    needsPaid: false,
    detail:
      "布・木・樹脂・紙などを実際に加工して現物を送る仕事です。" +
      "データでは納品できないので、あなたが手を動かして作り、梱包して発送することになります。",
    patterns: [
      // 素材や道具の名前が出るもの。データ納品と紛れない語を選ぶ。
      /(縫製|裁縫|刺繍|編み|木工|旋盤|溶接|塗装|研磨|造形|原型|鋳造|陶芸|レジン|樹脂|発泡ウレタン|石膏)/,
      // 現物の名前 + 作る
      /(衣装|コスプレ|フィギュア|ぬいぐるみ|アクセサリー|家具|什器|扇子|うちわ|トランプ|カード(?!決済|情報)|看板|のぼり|模型|プラモデル|ケース|額縁)[^。\n]{0,12}(製作|制作|作製|修復|加工|組み立て|作って|作成)/,
      /(ハンドメイド|手作り|自作)[^。\n]{0,10}(品|作品|商品|製作|制作|募集|依頼)/,
      // 完成品を送る、という書き方
      /(完成品|現物|実物)[^。\n]{0,10}(発送|郵送|お送り|納品)/,
      /(発送|郵送)[^。\n]{0,8}(していただ|お願い|対応)/,
    ],
  },
  {
    ...
    patterns: [
      ...
      /(?<![/／・|｜])(撮影|カメラマン|ロケ)/,
      // 「・」「、」の直後は、ページの職種タグ欄（「営業・接客」のような
      // カテゴリ一覧）であって仕事内容ではない。実データで、ホテル情報の
      // リスト作成（電話番号を集めるだけの在宅作業）が職種タグの
      // 「営業・接客」に当たって「現地作業」で落ちていた。
      /((?<![・、,])接客|配送|梱包|発送作業)/,
    ],
  },
];
