...
export async function stepTriage(ctx: StepContext): Promise<StepOutcome> {
  ...
  for (const lead of sorted) {
    ...
    // 応募にあなたが使う時間。取れなければ丸ごと損になるので原価に入れる。
    //
    // 以前は 0.5時間（30分）で置いていた。これは**自分で提案文を書く前提**の数字で、
    // 工程表で直したのと同じ間違いをここでも繰り返していた。
    // 提案文はこちらが下書きし、あなたがやるのは
    // 「要求項目の一覧と本文を突き合わせて、送るかどうか決める」だけ。
    //
    // 5分としているのは見積もりであって実測ではない。実際に応募を始めたら、
    // 承認までの実時間を記録して置き換えること。
    // ここが効くのは、単価の低い案件を数でこなす戦い方のとき。
    // 30分で見積もると、数をこなす道が計算上つぶれる。
    const PROPOSAL_HOURS = 5 / 60;
