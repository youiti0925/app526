...
・納期に遅れそうな場合は、遅れる前にご連絡します`,
...

  {
    ...
    manualMinutes: 120,
    caution:
      "顧客の会議音声・社内文書・個人情報など、外に出せない情報をここに貼らないでください。生成AIの無料枠は、入力が品質改善に使われ人が読む場合があると規約に明記されており、機密情報の送信は規約違反かつ守秘義務違反になります。機密を含む作業は、入力を学習に使わない有料プランかローカル実行に切り替えてください。\nまた、納品先によっては生成AIの使用自体に制限があります。主要なクラウドソーシングは「使ってもよいが隠してはいけない」という立場なので、使ったら申告してください。出力をそのまま納品せず、事実確認と自分の言葉での書き直しを必ず行ってください。",
    fields: [
